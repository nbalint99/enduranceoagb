...
class DetailsStintFragment : Fragment(), DetailsStintFragmentAdapter.DetailsStintFragmentItemClickListener, NewStintFragment.NewStintListener/*, NewBoxFragment.NewBoxListener*/ {
    ...
    override fun onCreateView(
        ...
        val binding = ActivityDetailsstintfragmentBinding.inflate(layoutInflater)

        val layoutManager: RecyclerView.LayoutManager = LinearLayoutManager(this.context)

        binding.rvMainDetailsStintFragment.setLayoutManager(layoutManager)

        adapter = DetailsStintFragmentAdapter(this)

        binding.rvMainDetailsStintFragment.adapter = adapter
        ...
        if (message == null) {
            handler.postDelayed(Runnable { layoutManager.smoothScrollToPosition(binding.rvMainDetailsStintFragment, null, 1) }, 100)
        }
        else {
            handler.postDelayed(Runnable { layoutManager.smoothScrollToPosition(binding.rvMainDetailsStintFragment, null, message.toInt()) }, 100)
        }

        binding.rvMainDetailsStintFragment.setItemViewCacheSize(16)
            ...
            if (p0.isSuccessful) {
                ...
                if (!p0.result.child("AllStint").child("numberOfStint").child(stintId.toString())
                    ...
                    for (element in p0.result.child("Teams").children) {
                        ...
                        val changePrev = "Etap: $prevStint"

                        val teamStintPrev = prevStint.toString() + "-" + element.child("Info").child("teamNumber").value.toString().toInt()

                        val prevAvgWeight = p0.result.child("Stints").child(changePrev).child("Info").child(teamStintPrev).child("prevAvgWeight").value.toString().toDoubleOrNull()
                        val prevDriverWeight = p0.result.child("Stints").child(changePrev).child("Info").child(teamStintPrev).child("driverWeight").value.toString().toDoubleOrNull()
                        val prevPlus = p0.result.child("Stints").child(changePrev).child("Info").child(teamStintPrev).child("plusWeight").value.toString().toDoubleOrNull()
                        val prevDriverTotal = prevPlus?.let { prevDriverWeight?.plus(it) }
                        val prevTotalWeight = prevDriverTotal?.let { prevAvgWeight?.plus(it) }

                        val stint = Stint(element.child("Info").child("nameTeam").value.toString(),
                            element.child("Info").child("teamNumber").value.toString().toInt(),
                            null, stintId.toString().toInt(), element.child("Info").child("shortTeamName").value.toString(),null, null, null, false, prevTotalWeight)
                        ...
                        if (teamNumber == iterator) {
                            if (stintId.toString().toInt() == 1) {
                                ...
                                for (ready in hasStintDone) {
                                    ...
                                    val prevAvgWeight =
                                        p0.result.child("Stints").child(changePrevStint).child("Info").child(changePrev).child("prevAvgWeight").value.toString().toDoubleOrNull()
                                    val prevDriverWeight = p0.result.child("Stints").child(changePrevStint).child("Info").child(changePrev).child("driverWeight").value.toString().toDoubleOrNull()
                                    val prevDriverTotal = prevDriverWeight?.let {
                                        prevPlusWeight?.plus(
                                            it
                                        )
                                    }
                                    val prevTotal = prevAvgWeight?.let { prevDriverTotal?.plus(it) }
                                        ...
                                        if (teamNum.toString().toIntOrNull() != null) {
                                            ...
                                            if (teamNum == teamNumber && stintNum == stintId.toString()
                                                ...
                                                val addStint = Stint(
                                                    ...
                                                    info,
                                                    prevInfo,
                                                    hasStintD, /*prevAvgWeight*/ prevTotal, driverWeight,
        ...
        dbRef.get().addOnCompleteListener { p0 ->
            if (p0.isSuccessful) {
                val numberOfTeamsAll =
                    p0.result.child("Info").child("numberOfTeams").value.toString().toInt()
                if (changeTime(numberOfTeamsAll, stintId.toInt()) == 0) {
                ...
                if (stintId.toString().toInt() == 1) {
                    //println("?ide????1?")
                    //val firstTotalWeight = driverWeight?.plus(weight)
                    val stint = Stint(
                        teamName,
                        teamNumber,
                        driver,
                        ...
                        if (firstMore == true) {
                            if (numberOfTeams < 12) {
                                ...
                                if (teamNumber == numberOfTeams) {
                                    ...
                                    val teamStintNext12 = "${stintId.toString().toInt() + 1}-box12"
                                    dbRef.child("Stints").child(changeNext).child("Info")
                                        .child(teamStintNext11).child("kartNumber")
                                        .setValue(kartNumber)
                                    dbRef.child("Stints").child(changeNext).child("Info")
                                        .child(teamStintNext12).child("kartNumber")
                                        .setValue(kartNumber)
                                }
                                if (teamNumber == secondGroupLast) {
                                    ...
                                    dbRef.child("Stints").child(changeNext).child("Info")
                                        .child(teamStintNext21).child("kartNumber")
                                        .setValue(kartNumber)
                                    val prevBoxKart =
                                        p0.result.child("Stints").child(change).child("Info")
                                            .child(teamStintNext21)
                                            .child("kartNumber").value.toString().toIntOrNull()
                                    dbRef.child("Stints").child(changeNext).child("Info")
                                        .child(teamStintNext22).child("kartNumber")
                                        .setValue(prevBoxKart)
                                    dbRef.child("Stints").child(changeNext2).child("Info")
                                        .child(teamStintNext221).child("kartNumber")
                                        .setValue(kartNumber)
                                }
                            }
                        }
